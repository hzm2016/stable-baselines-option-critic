...
import gym
from stable_baselines.common.distributions import make_pdtype
import numpy as np
import pdb
...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True,
        ...
        for i in range(num_hid_layers):
            last_out = tf.nn.tanh(tf.layers.dense(last_out, hid_size, kernel_initializer=U.normc_initializer(1.0), name="vffc%i"%(i+1)))

        self.vpred = dense3D2(last_out, 1, "vffinal", option,
                              num_options=num_options, weight_init=U.normc_initializer(1.0))[:, 0]

        self.tpred = tf.nn.sigmoid(dense3D2(tf.stop_gradient(last_out), 1, "termhead", option,
                                            num_options=num_options, weight_init=U.normc_initializer(1.0)))[:, 0]
